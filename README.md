...
This will also output the portfolio positions of the strategy. We have provided default values to the parameters of the market instruments (namely, the underlying stock and the vanilla options) so that the focus is on the qualitative behaviour. The following code shows how one can tweak these values if desired.
